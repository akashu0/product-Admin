  ...
  return (
    <nav className="bg-blue-900 text-white fixed top-0 left-0 w-full h-16 z-10 flex items-center justify-between px-4 sm:px-6 md:px-10">
      ...
      <div className="flex items-center space-x-6">
        ...
        <div
          className="hidden md:flex items-center space-x-1 cursor-pointer"
          onClick={openModal}
        >
      ...
      <div className="flex-1 hidden lg:flex items-center justify-center">
        <div className="relative flex-1 max-w-lg">
          <input
            type="text"
            className="w-full h-10 pl-10 pr-4 rounded-lg bg-blue-900 border text-white text-lg focus:outline-none"
            placeholder="Search.."
          />
          <svg
            className="absolute left-3 top-1/2 transform -translate-y-1/2 text-white w-6 h-6"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
          >
        ...
        {user && userType === "user" ? (
          <div className="ml-12 md:ml-4 lg:ml-12 w-32 md:w-40 lg:w-32 h-14 rounded-lg pt-1">
            <Link to={user ? "/admin/addcompany" : "/joinprolio"}>
              <button className="w-full h-full text-blue-900 font-semibold bg-blue-50 rounded-lg hover:bg-blue-100 transition duration-300 ease-in-out">
                Join Prolio
              </button>
            </Link>
          </div>
